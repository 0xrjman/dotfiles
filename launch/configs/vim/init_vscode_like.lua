...
local config = {
    ...
    options = { -- set vim options here (vim.<first_key>.<second_key> =  value)
        ...
        g = {
            ...
            jump_word_e = function(v) -- meh
                ...
                vim.api.nvim_input((vim.api.nvim_get_current_line():sub(col + 1, col + 1) == '.' and '<right>' or '') ..
                                       (v and '' or '<C-o>') .. 'e<right>')
            end,

            jump_word_b = function(v) -- meh v.2 | Added: repeating code, lol | Better than having multiple local variables and an if statment
                local col = vim.api.nvim_win_get_cursor(vim.api.nvim_get_current_win())[2]
                vim.api.nvim_input((vim.api.nvim_get_current_line():sub(col, col) == '.' and '<left>' or '') ..
                                       (v and '' or '<C-o>') .. 'b')
    ...
    plugins = { -- Configure plugins
        ...
        {'mg979/vim-visual-multi'}, {'m-pilia/vim-smarthome'}, {'projekt0n/github-nvim-theme'}, {'fcpg/vim-orbital'},
        {'romainl/vim-malotru'}, {'tpope/vim-sleuth'}, {'tpope/vim-commentary'}, {'Mofiqul/vscode.nvim'}, {
        ...
        cmp = function(config)
            ...
            config.enabled = function() -- https://www.reddit.com/r/neovim/comments/skkp1r/disable_cmp_inside_comments/
                ...
                return cmp.visible() or
                           not (context.in_treesitter_capture("comment") or context.in_treesitter_capture('string') or
        ...
        ["null-ls"] = function(config)
            ...
            config.sources = {null_ls.builtins.formatting.rufo, -- Set a formatter
    ...
    polish = function() -- This function is run last -- good place to configure mappings and vim options
        ...
        vim.api.nvim_command(
            "cnoremap <expr> <CR> (getcmdtype() ==# ':' && getcmdline() =~# '^call go_to_relative_line') ? '<CR><C-o>^' : '<CR>'") -- https://noahfrederick.com/log/vim-streamlining-grep
